...
public class IBChangePasswordTestM extends BaseClass{
	@Test(groups = {"regression", "minor"})
	public void ibChangePassword() {
		   //test data
		...
		openaccPg.sendValue(com.banking.enumFiles.CreateAccAttr.NAME, name_);
	...
	//	waitUtility.waitUntilPresenceOfAlert();
		...
		soft.assertEquals(custWelcomeMsg, "Welcome "+name_, "customer profile not displayed");
		
		custIBPg.logoutBtnAction();
		
		iblgnPage.ibLoginAction(custID, map.get("password"));
		custIBPg.changePasswordTabAction();
		changPwdPg.changePasswordAction(map.get("oldpassword"), map.get("confirmpassword"), map.get("newpassword"));
		popupUtility.acceptAlert(driver);
		custIBPg.logoutBtnAction();
		iblgnPage.ibLoginAction(custID, map.get("newpassword"));
		String acwlc = custIBPg.welomeMsgAction();
		String exwlc = "Welcome "+name_;
		soft.assertEquals(acwlc, exwlc, "actual and expected not matched");
		soft.assertAll();
}
}